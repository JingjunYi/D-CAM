...
if __name__ == '__main__':
    ...
    root = 'F:/dataset/Med/HistoSeg/WSSS4LUAD'
    new_root = 'F:/dataset/Med/HistoSeg/WSSS4LUAD_dg'
    ...
    for img in tqdm(os.listdir(os.path.join(root, 'train'))):
        ...
        new_label_str = '['+label_str[0]+label_str[3]+']'
        origin_img = os.path.join(root, 'train', img)
        new_img = os.path.join(new_train, img.replace('['+label_str+']', new_label_str))
        img_ = cv2.resize(cv2.imread(origin_img), (224, 224), interpolation=cv2.INTER_NEAREST)
        cv2.imwrite(new_img, img_)
    ...
    for mask in tqdm(os.listdir(os.path.join(root, 'val', 'mask'))):
        label_ = Image.open(os.path.join(root, 'val', 'mask', mask))
        label_ = np.array(label_).astype(np.uint8)
        label = cv2.resize(label_, (224, 224), interpolation=cv2.INTER_NEAREST)
