...
class Trainer(object):
    ...
    def validation(self, epoch):
        ...
        if (epoch+1) % 2 == 0:
            self.saver.save_checkpoint({
                'state_dict': self.model.module.state_dict(),
                'optimizer': self.optimizer.state_dict()
            }, 'stage2_checkpoint_e{}_{}.pth'.format(epoch, mIoU))
    ...
    def test(self, epoch, Is_GM):
        ...
        for i, sample in enumerate(tbar):
            ...
            visualimg  = Image.fromarray(pred.astype(np.uint8))
            visualimg.putpalette(palette)
            visualimg.save(os.path.join('vis/l2w', str(i+1)+'.png'), format='PNG')
            # visualmask  = Image.fromarray(target.astype(np.uint8))
            # visualmask.putpalette(palette)
            # visualmask.save(os.path.join('vis/l2w/mask', str(i+1)+'.png'), format='PNG')
            if i == 49:
                exit()
        # Acc = self.evaluator.Pixel_Accuracy()
        # Acc_class = self.evaluator.Pixel_Accuracy_Class()
