...
def scores(label_trues, label_preds, n_class):
    ...
    for lt, lp in zip(label_trues, label_preds):
        lp[lt==n_class]=n_class
        tmp = _fast_hist(lt.flatten(), lp.flatten(), n_class)
        hist += tmp
    hist[n_class,n_class]=0
    acc = np.diag(hist).sum() / hist.sum()
    acc_cls = np.diag(hist)[0:n_class] / hist.sum(axis=1)[0:4]
    acc_cls = np.nanmean(acc_cls)
    iu = np.diag(hist)[0:n_class] / ((hist.sum(axis=1) + hist.sum(axis=0) - np.diag(hist))[0:4]) #true和pred都没有，则nan；true有pred没有或者true没有pred有，则zero
    mean_iu = np.nanmean(iu) #gt中有的其实就不会存在nan了
    freq = hist.sum(axis=1)[0:n_class] / hist.sum() #groundtrue中每一个类的占比
